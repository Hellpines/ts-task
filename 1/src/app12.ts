...
function CountOfUnique(str: string) : string{
    ...
    for(let i = 0; i < str1.length; i++){
        if(!res1.includes(str1[i])) res1.push(str1[i]);
    }
    for(let i = 0; i < str2.length; i++){
        if(!res2.includes(str2[i])) res2.push(str2[i]);
    }
    if(res1.length === res2.length) return "Можно"
    return "Нельзя"
}
console.log(CountOfUnique("aabbcc"))